...
** Commands **
* /alertmaster popup pins the alert window open so it is not auto-closed.
* /alertmaster config will toggle the config window.
** Alert Window **
...
local CharConfig                                                                                                          = 'Config'
local CharCommands                                                                                                        = 'Commands'
local newConfigFile                                                                                                       = string.format("%s/MyUI/AlertMaster/%s/Settings.lua",
    mq.configDir, Module.Server)
local defaultConfig = {
    delay = 1,
    remind = 30,
    remindNPC = 5,
    ...
    doSoundNPC = true,
    doSoundGM = true,
    doSoundPCEntered = true,
    doSoundPCLeft = true,
    soundPC = 'PC.wav',
    soundNPC = 'NPC.wav',
    soundGM = 'GM.wav',
    soundPCEntered = 'PCEntered.wav',
    soundPCLeft = 'PCLeft.wav',
    volPC = 50,
    volNPC = 50,
    volGM = 100,
    volPCEntered = 100,
...
local tSafeZones, spawnAlerts, spawnsSpawnMaster, settings, trackedSpawns                                                 = {}, {}, {}, {}, {}
...
local doBeep, doAlert, haveSM, importZone, doSoundNPC, doSoundGM, doSoundPC, forceImport, doSoundPCEntered, doSoundPCLeft = false, false, false, false, false, false,
    false, false, false, false, false
local delay, remind, pcs, spawns, gms, announce, ignoreguild, radius, zradius, remindNPC                                  = 1, 30, true, true, true, false, true, 100,
    100, 5
-- [[ UI ]] --
local AlertWindow_Show, AlertWindowOpen, showTooltips, active                                                             = false, false, true, false
...
local function refresh_db_settings()
    local db = Module:OpenDB()
    settings.SafeZones = Module:GetSafeZones(db)
    settings.Ignore = Module:GetIgnoredPlayers(db)
    if db then db:close() end
    tSafeZones = {}
    for _, zoneShort in ipairs(settings.SafeZones) do tSafeZones[zoneShort] = true end
end

local function refresh_tracked_spawns()
    local db = Module:OpenDB()
    Module.TempSettings.NpcList = Module:GetSpawns(Zone.ShortName(), db) or {}
    if db then db:close() end
    trackedSpawns = {}
    for _, spawnName in ipairs(Module.TempSettings.NpcList) do trackedSpawns[spawnName] = true end
end
...
local function load_settings()
    ...
    refresh_db_settings()
    refresh_tracked_spawns()
    set_settings()
    save_settings()
end

-- PCs stop sending position updates past 600 units, so anything beyond that is a last-known value
local function ColorPlayerDistance(distance)
    if distance > 600 then
        return Module.Colors.color('grey')
...
---@param spawn MQSpawn
local function SpawnToEntry(spawn, id)
...
local function InsertTableSpawn(dataTable, spawn, id, opts)
    if spawn then
        local entry = SpawnToEntry(spawn, id)
        if opts then
...
local function should_include_player(name, guild, groupCount, raidCount)
    for _, ignored in pairs(settings['Ignore'] or {}) do
        if ignored == name then return false end
    end
    if groupCount > 0 and Group.Member(name).Index() ~= nil then return false end
    if raidCount > 0 and Raid.Member(name)() ~= nil then return false end
    if ignoreguild and Module.Guild == guild then return false end
    return true
end
...
local function collect_players()
    ...
    local groupCount = Group.Members() or 0
    local raidCount = Raid.Members() or 0
    local now = os.time()
    local zoneWide, inRadius, gmPlayers = {}, {}, {}
    local found = mq.getFilteredSpawns(function(spawn)
        if (spawn.ID() or 0) == myId then return false end
        return spawn.Type() == 'PC' or (spawn.GM() or false)
    end)
    numDisplayPlayers = 0
    for i = 1, #found do
        local spawn = found[i]
        local name = spawn.DisplayName()
        local guild = spawn.Guild() or 'No Guild'
        if name and should_include_player(name, guild, groupCount, raidCount) then
            local isGM = spawn.GM() or false
            local entry = {
                name = (isGM and 'g*GM*x ' or '') .. 'r' .. name .. 'x',
                tblName = name,
                level = spawn.Level() or 0,
                guild = '<y' .. guild .. 'x>',
                tblGuild = guild,
                distance = math.floor(spawn.Distance3D() or 0),
                time = now,
                isGM = isGM,
            }
            zoneWide[name] = entry
            numDisplayPlayers = numDisplayPlayers + 1
            if isGM then gmPlayers[name] = entry end
            if entry.distance <= radius and (spawn.DistanceU() or 0) <= zradius then
                inRadius[name] = entry
            end
        end
    end
    displayTablePlayers = zoneWide
    sortedPlayers = {}
    for _, entry in pairs(zoneWide) do sortedPlayers[#sortedPlayers + 1] = entry end
    table.sort(sortedPlayers, function(a, b) return a.distance < b.distance end)
    return zoneWide, inRadius, gmPlayers
end

local function spawn_search_npcs()
    local tmp = {}
    if next(trackedSpawns) == nil then return tmp end
    local found = mq.getFilteredSpawns(function(spawn) return spawn.Type() == 'NPC' end)
    for i = 1, #found do
        local spawn = found[i]
        -- Case-sensitive comparison using CleanName for exact matching
        if trackedSpawns[spawn.DisplayName()] or trackedSpawns[spawn.Name()] then
            tmp[spawn.ID()] = spawn
        end
    end
    return tmp
...
local function check_for_gms(tmp)
...
local function check_for_pcs(tmp)
...
local function check_for_spawns()
    if active then
        ...
        if tmp ~= nil and spawns then
            ...
            elseif not alwaysShow and (numDisplayPlayers == 0 or check_safe_zone()) then
                AlertWindow_Show = false
                AlertWindowOpen = false
            end
            if doAlert and numDisplayPlayers > lastPlayerCount and not check_safe_zone() then
                AlertWindow_Show = true
                AlertWindowOpen = true
            end
            lastPlayerCount = numDisplayPlayers
        end
    end
end

local function check_for_announce(tmp)
...
local function check_for_zone_change()
    ...
    if active and (zone_id == nil or zone_id ~= Zone.ID()) then
        ...
        tGMs, tAnnounce, tPlayers, tSpawns, spawnAlerts, Table_Cache.Alerts = {}, {}, {}, {}, {}, {}
        zone_id = Zone.ID()
        alertTime = os.time()
        doOnce = true
        displayTablePlayers = {}
        ...
        if ImGui.CollapsingHeader("Commands") then
            if ImGui.BeginTable("CommandTable", 2, ImGuiTableFlags.Resizable) then
                ...
                for key, command in pairs(Module.Settings[CharCommands]) do
                    ...
                    if tmpCmd ~= command and ImGui.IsItemDeactivatedAfterEdit() then
...
local function BuildPlayerRows() -- Players sharing the zone, nearest first
    if ImGui.BeginTable("PlayerTable", 4, bit32.bor(ImGuiTableFlags.Resizable, ImGuiTableFlags.ScrollY, ImGuiTableFlags.NoBordersInBodyUntilResize)) then
        ImGui.TableSetupScrollFreeze(0, 1)
        ImGui.TableSetupColumn("Name", ImGuiTableColumnFlags.WidthFixed, 110)
        ImGui.TableSetupColumn("Lvl", ImGuiTableColumnFlags.WidthFixed, 30)
        ImGui.TableSetupColumn("Dist", ImGuiTableColumnFlags.WidthFixed, 50)
        ImGui.TableSetupColumn("Guild", ImGuiTableColumnFlags.WidthStretch)
        ImGui.TableHeadersRow()
        local clipper = ImGuiListClipper.new()
        clipper:Begin(#sortedPlayers)
        while clipper:Step() do
            for i = clipper.DisplayStart, clipper.DisplayEnd - 1, 1 do
                local player = sortedPlayers[i + 1]
                ImGui.TableNextRow()
                ImGui.TableSetColumnIndex(0)
                ImGui.PushStyleColor(ImGuiCol.Text, Module.Colors.color(player.isGM and 'yellow' or 'red'))
                ImGui.Text(player.tblName)
                ImGui.PopStyleColor()
                ImGui.TableSetColumnIndex(1)
                ImGui.Text('%s', tostring(player.level))
                ImGui.TableSetColumnIndex(2)
                ImGui.PushStyleColor(ImGuiCol.Text, ColorPlayerDistance(player.distance))
                ImGui.Text('%s', tostring(player.distance))
                ImGui.PopStyleColor()
                ImGui.TableSetColumnIndex(3)
                ImGui.Text(player.tblGuild)
            end
        end
        clipper:End()
        ImGui.EndTable()
...
function DrawAlertGUI() -- Draw GUI Window
    if AlertWindowOpen then
        ...
        if show then
            ImGui.SetWindowFontScale(ZoomLvl)
            if ImGui.BeginTabBar('AlertTabs') then
                if ImGui.BeginTabItem('Spawns') then
                    BuildAlertRows()
                    ImGui.EndTabItem()
                end
                if numDisplayPlayers > 0 and not check_safe_zone() then
                    if ImGui.BeginTabItem(string.format('Players (%d)###PlayersTab', numDisplayPlayers)) then
                        BuildPlayerRows()
                        ImGui.EndTabItem()
                    end
                end
...
local function load_binds()
    local function bind_alertmaster(cmd, val)
        ...
        if cmd == 'popup' then
            ...
            AlertWindowOpen = alwaysShow
            AlertWindow_Show = alwaysShow
            Module.Utils.PrintOutput('AlertMaster', nil, alwaysShow and '\ayPinning Alert Window open.' or '\ayUnpinning Alert Window.')
        end
        if cmd == 'config' then
            openConfigGUI = not openConfigGUI
            Module.Utils.PrintOutput('AlertMaster', nil, openConfigGUI and '\ayShowing Config Window.' or '\ayClosing Config Window.')
        end
        ...
        elseif cmd == 'spawnlist' then
            ...
            for _, spawnName in ipairs(Module.TempSettings.NpcList) do
                local up = false
                for _, tracked in pairs(tSpawns) do
                    if tracked ~= nil and (tracked.DisplayName == spawnName or (tracked.Spawn and tracked.Spawn.Name() == spawnName)) then
                        up = true
                        break
                    end
                end
                if up then
                    Module.Utils.PrintOutput('AlertMaster', nil, string.format('\ag[Live] %s\ax', spawnName))
                else
                    Module.Utils.PrintOutput('AlertMaster', nil, string.format('\a-t[Dead] %s\ax', spawnName))
                end
            end
            -- else
        ...
        if cmd == nil or cmd == 'help' then
            ...
            Module.Utils.PrintOutput('AlertMaster', nil, '\t\ay/alertmaster popup\a-t -- Pins the Alert Window open (ignores auto-close)')
            Module.Utils.PrintOutput('AlertMaster', nil, '\t\ay/alertmaster config\a-t -- Toggles Display of Config Window')
...
local function setup()
    ...
    local zoneWide, inRadius, gmPlayers = collect_players()
    check_for_pcs(inRadius)
    check_for_gms(gmPlayers)
    check_for_announce(zoneWide)
    check_for_spawns()
    if not loadedExeternally then
